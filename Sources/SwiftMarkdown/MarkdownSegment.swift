...
//  through headings, prose, lists and quotes. Code blocks and tables are pulled out
//  because they scroll horizontally, which a shared text container cannot express — see
//  MarkdownCodeBlockView for why that trade is made in their favour.
//
...
public enum MarkdownSegment: Identifiable {
    ...
    /// A table, rendered on its own so it can scroll past the available width
    /// instead of wrapping its rows.
    case table(id: Int, table: MarkdownTable)

    public var id: Int {
        switch self {
        case .prose(let id, _), .code(let id, _, _), .table(let id, _): return id
        }
    }

    /// Whether this segment is one of the kinds that leaves the shared text storage.
    ...
    private static func split(_ blocks: [MarkdownBlock],
        ...
        // Prose runs to the next split-out block, so a boundary is only real with code or
        // a table on one side of it — stopping mid-run would fuse differently and split a
        // selection that should have been continuous.
        if let previous {
            for (index, segment) in previous.segments.enumerated() {
                ...
                let cutIsReal = segment.isStandalone
                    || (range.upperBound < blocks.count && blocks[range.upperBound].isStandalone)
        ...
        while i < blocks.count {
            ...
            case .table(let table):
                flushProse(upTo: i)
                segments.append(.table(id: segments.count, table: table))
                ranges.append(i..<(i + 1))
                pendingStart = i + 1
                proseTail = nil
...
private extension MarkdownBlock {
    /// Blocks that get a view of their own instead of joining a prose run.
    var isStandalone: Bool {
        switch self {
        case .codeBlock, .table: return true

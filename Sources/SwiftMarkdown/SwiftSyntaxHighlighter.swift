...
//  That is enough for the code blocks in streamed output, which run well under a
...
private func dynamic(light: String, dark: String) -> PlatformColor {
    ...
    return NSColor(name: nil) { appearance in
        appearance.bestMatch(from: [.aqua, .darkAqua]) == .darkAqua
            ? PlatformColor(markdownHex: dark) : PlatformColor(markdownHex: light)
    }
    #else
    return UIColor { traits in
        traits.userInterfaceStyle == .dark ? PlatformColor(markdownHex: dark) : PlatformColor(markdownHex: light)
    }
    #endif
}

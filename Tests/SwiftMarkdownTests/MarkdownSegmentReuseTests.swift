...
struct MarkdownSegmentReuseTests {
    ...
    private var style: MarkdownStyle {
        MarkdownStyle.standard(appearanceIsDark: true)
    }
        ...
        for line in source.components(separatedBy: "\n") {
            ...
            for (a, b) in zip(incremental.segments, cold) {
                ...
                case (.table(let ia, let ta), .table(let ib, let tb)):
                    #expect(ia == ib, sourceLocation: sourceLocation)
                    #expect(ta == tb, sourceLocation: sourceLocation)
                default:
    ...
    func styleChangeInvalidates() {
        ...
        let dark = MarkdownSegment.split(document, style: MarkdownStyle.standard(appearanceIsDark: true),
                                         reusing: nil)

        let lightStyle = MarkdownStyle.standard(appearanceIsDark: false)
